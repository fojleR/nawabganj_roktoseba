...
class _BodyState extends State<Body> {
  ...
  void updateDistrict(String value){
    setState(() {
      ...
      upazilla = '';
      union = '';
      gram = ''; // Clear district dropdown items
  ...
  void fetchDonorData(String bloodGroup, String upazilla, String union, String gram) async {
    QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('Users')
        ...
        .where('presentUpazilla', isEqualTo: upazilla)
    // .where('presentUnion', isEqualTo: union)
    // .where('presentVillage', isEqualTo: gram)
        .get();

    List<Map<String, String>> donorInfo = [];

    setState(() {
      donorData = snapshot.docs;
      print(donorData);
    });
    ...
    if (donorData.isEmpty) {
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text("No donors found matching the specified criteria."),
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.of(context).pop();
      ...
      Navigator.push(
        context,
        MaterialPageRoute(builder: (context) => CardList(donorInfo: donorInfo)),
      );
    }
  }
